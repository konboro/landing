// Feedback + status components — Badge, ProgressBar, SocPill, Banner, Sheet.
import React from 'react';
import {
  View,
  Modal,
  Pressable,
  StyleSheet,
  type ViewStyle,
  type StyleProp,
} from 'react-native';
import { socColor } from '@penny/ui';
import { useTheme, makeStyles } from '../../brand';
import type { RiderTheme } from '../../brand';
import { T, Row } from './primitives';
import { Icon, type IconName } from './Icon';

type Tone = 'neutral' | 'primary' | 'success' | 'warning' | 'danger';

function toneBg(theme: RiderTheme, tone: Tone): string {
  switch (tone) {
    case 'neutral': return theme.color.surfaceAlt;
    case 'primary': return theme.color.primarySoft;
    case 'success': return theme.color.successSoft;
    case 'warning': return theme.color.warningSoft;
    case 'danger': return theme.color.dangerSoft;
  }
}
function toneFg(theme: RiderTheme, tone: Tone): string {
  switch (tone) {
    case 'neutral': return theme.color.textMuted;
    case 'primary': return theme.color.primary;
    case 'success': return theme.color.success;
    case 'warning': return theme.color.warning;
    case 'danger': return theme.color.danger;
  }
}

export function Badge({
  label,
  tone = 'neutral',
  icon,
  style,
}: {
  label: string;
  tone?: Tone;
  icon?: IconName;
  style?: StyleProp<ViewStyle>;
}) {
  const theme = useTheme();
  const styles = useStyles(theme);
  const fg = toneFg(theme, tone);
  return (
    <View style={[styles.badge, { backgroundColor: toneBg(theme, tone) }, style]}>
      {icon ? <Icon name={icon} size={12} color={fg} /> : null}
      <T variant="caption" color={fg} style={{ fontWeight: '700' }}>
        {label}
      </T>
    </View>
  );
}

export function ProgressBar({
  value,
  tone = 'primary',
  height = 8,
}: {
  value: number; // 0..1
  tone?: Tone;
  height?: number;
}) {
  const theme = useTheme();
  const styles = useStyles(theme);
  const pct = Math.max(0, Math.min(1, value));
  return (
    <View style={[styles.track, { height, borderRadius: height / 2 }]}>
      <View
        style={{
          width: `${pct * 100}%`,
          height: '100%',
          borderRadius: height / 2,
          backgroundColor: toneFg(theme, tone),
        }}
      />
    </View>
  );
}

/** Battery/state-of-charge pill with the shared soc color logic. */
export function SocPill({ soc, size = 'md' }: { soc: number; size?: 'sm' | 'md' }) {
  const theme = useTheme();
  const styles = useStyles(theme);
  const level = socColor(soc);
  const tone: Tone = level === 'ok' ? 'success' : level === 'warn' ? 'warning' : 'danger';
  const fg = toneFg(theme, tone);
  return (
    <View style={[styles.badge, { backgroundColor: toneBg(theme, tone) }, size === 'sm' && { paddingVertical: 2 }]}>
      <Icon name="battery" size={12} color={fg} />
      <T variant="caption" color={fg} style={{ fontWeight: '700' }}>
        {Math.round(soc)}%
      </T>
    </View>
  );
}

export function Banner({
  tone = 'primary',
  icon,
  title,
  body,
  action,
  style,
}: {
  tone?: Tone;
  icon?: IconName;
  title: string;
  body?: string;
  action?: React.ReactNode;
  style?: StyleProp<ViewStyle>;
}) {
  const theme = useTheme();
  const styles = useStyles(theme);
  const fg = toneFg(theme, tone);
  return (
    <View style={[styles.banner, { backgroundColor: toneBg(theme, tone), borderColor: fg }, style]}>
      <Row gap={theme.space.md} align="flex-start">
        {icon ? <Icon name={icon} size={20} color={fg} /> : null}
        <View style={{ flex: 1 }}>
          <T variant="body" color={fg} style={{ fontWeight: '700' }}>
            {title}
          </T>
          {body ? (
            <T variant="caption" color={fg} style={{ marginTop: 2 }}>
              {body}
            </T>
          ) : null}
        </View>
        {action}
      </Row>
    </View>
  );
}

export function Sheet({
  visible,
  onClose,
  children,
  title,
  dismissable = true,
}: {
  visible: boolean;
  onClose: () => void;
  children: React.ReactNode;
  title?: string;
  dismissable?: boolean;
}) {
  const theme = useTheme();
  const styles = useStyles(theme);
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <Pressable style={styles.scrim} onPress={dismissable ? onClose : undefined}>
        <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
          <View style={styles.grabber} />
          {title ? (
            <Row justify="space-between" style={{ marginBottom: theme.space.md }}>
              <T variant="subtitle">{title}</T>
              {dismissable ? (
                <Pressable onPress={onClose} hitSlop={12}>
                  <Icon name="close" size={20} color={theme.color.textMuted} />
                </Pressable>
              ) : null}
            </Row>
          ) : null}
          {children}
        </Pressable>
      </Pressable>
    </Modal>
  );
}

const useStyles = makeStyles((t) => ({
  badge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 5,
    paddingHorizontal: 9,
    paddingVertical: 4,
    borderRadius: t.radius.pill,
    alignSelf: 'flex-start',
  },
  track: {
    width: '100%',
    backgroundColor: t.color.surfaceAlt,
    overflow: 'hidden',
  },
  banner: {
    borderRadius: t.radius.lg,
    borderWidth: StyleSheet.hairlineWidth,
    padding: t.space.lg,
  },
  scrim: {
    flex: 1,
    backgroundColor: t.color.scrim,
    justifyContent: 'flex-end',
  },
  sheet: {
    backgroundColor: t.color.surface,
    borderTopLeftRadius: t.radius.xl,
    borderTopRightRadius: t.radius.xl,
    padding: t.space.xl,
    paddingBottom: t.space.xxl,
    gap: t.space.sm,
  },
  grabber: {
    alignSelf: 'center',
    width: 40,
    height: 4,
    borderRadius: 2,
    backgroundColor: t.color.border,
    marginBottom: t.space.md,
  },
}));
